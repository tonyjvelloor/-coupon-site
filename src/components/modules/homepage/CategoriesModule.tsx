    ...
    return (
        <section className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-20 transition-colors duration-300">
            <div className="flex justify-between items-end mb-10">
                <div>
                    <h2 className="text-3xl font-display-lg font-bold text-slate-900 tracking-tight">Trending Right Now</h2>
                    <p className="text-lg text-slate-500 mt-2">Everyone is buying from these top categories today.</p>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                {categories.map((cat) => (
                    <Link key={cat.id} href={`/best/${cat.slug}-coupons`} className="group relative bg-white p-6 rounded-2xl border border-slate-200 hover:border-brand-indigo/30 shadow-premium-sm hover:shadow-premium-md transition-all duration-300 active:scale-[0.98] flex flex-col items-start gap-4 overflow-hidden">
                        ...
                        <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:scale-110 transition-transform duration-300 relative z-10">
                            <span className="material-symbols-outlined text-slate-600 group-hover:text-brand-indigo transition-colors" style={{ fontVariationSettings: "'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24" }}>
                                {getIconForCategory(cat.slug)}
                            </span>
                        </div>
                        
                        <div className="relative z-10">
                            <span className="block text-base font-semibold text-slate-900 group-hover:text-brand-indigo transition-colors mb-1">
                                {cat.name}
                            </span>
