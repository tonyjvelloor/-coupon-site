    ...
    if (!isOpen) {
        return (
            <button 
                ...
                className="w-full flex items-center justify-between p-4 mt-2 bg-surface-50 hover:bg-surface-100 rounded-xl border border-surface-200 text-merchant-900 transition font-medium"
            >
    ...
    return (
        <div className="mt-2 bg-white rounded-xl border border-surface-200 overflow-hidden shadow-sm">
            <button 
                ...
                className="w-full flex items-center justify-between p-4 bg-surface-50 border-b border-surface-200 text-merchant-900 transition font-medium hover:bg-surface-100"
            >
            ...
            <div className="p-6 space-y-6">
                <div>
                    <label className="block text-sm font-bold text-merchant-900 mb-2">Cart Value (₹)</label>
                ...
                <div className="space-y-3 pt-2 border-t border-surface-100">
                    <div className="flex justify-between text-sm">
                        ...
                        <span className="font-bold text-merchant-900">₹{calculatedBase.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                        <span className="text-surface-600 font-medium">Cashback (5%)</span>
                        <span className="font-bold text-merchant-900">₹{calculatedCashback.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                        <span className="text-surface-600 font-medium">Card Offer (2.5%)</span>
                        <span className="font-bold text-merchant-900">₹{calculatedCard.toLocaleString()}</span>
                    </div>
                </div>

                <div className="pt-4 border-t-2 border-dashed border-surface-200 flex justify-between items-center">
                    <span className="font-bold text-merchant-900 uppercase tracking-widest text-sm">Total Saved</span>
