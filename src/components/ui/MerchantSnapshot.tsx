...
export function MerchantSnapshot({ store }: MerchantSnapshotProps) {
    ...
    return (
        <Card interactive className="h-full border-surface-200 dark:border-surface-800 hover:border-primary dark:hover:border-primary transition-colors duration-300">
            <Link href={`/stores/${store.slug}`} className="flex flex-col h-full group p-5">
                ...
                <div className="space-y-3 mb-4">
                    <div className="flex items-center justify-between text-sm">
                        <span className="text-surface-600 dark:text-surface-400 font-medium">Working Offers</span>
                        <span className="font-bold text-slate-900 dark:text-surface-100">{store.offerCount} Active</span>
                    </div>
                    {store.cashbackRate && (
                        <div className="flex items-center justify-between text-sm">
                            <span className="text-surface-600 dark:text-surface-400 font-medium">Cashback</span>
                            <span className="font-bold text-green-600 dark:text-green-400">{store.cashbackRate}</span>
                        </div>
                    )}
                    <div className="flex items-center justify-between text-sm">
                        <span className="text-surface-600 dark:text-surface-400 font-medium">Updated</span>
                        <span className="font-bold text-surface-900 dark:text-surface-100">{store.lastVerified || 'Today'}</span>
                    </div>
                </div>

                {/* Badges / Checked Capabilities */}
                <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-surface-100 dark:border-surface-800">
                    <span className="inline-flex items-center gap-1 px-2 py-1 bg-surface-50 dark:bg-surface-800 text-surface-600 dark:text-surface-400 text-[10px] font-bold uppercase rounded-md border border-surface-200 dark:border-surface-700">
